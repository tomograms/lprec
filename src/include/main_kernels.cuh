...
__device__ float linearTex2D(cudaTextureObject_t tex, float x, float y, float z, int N0,int N1)
{
	float2 t0;
	t0.x = x/(float)N0;
	t0.y = y/(float)N1;
	return tex2DLayered<float>(tex, t0.x, t0.y, (int)z);
}

//cubic interpolation via two linear interpolations for several slices, texture is not normalized
__device__ float cubicTex2D(cudaTextureObject_t tex, float x, float y, float z, int N0,int N1)
{
	...
	t0.y = h0.y/(float)N1;
	t1.y = h1.y/(float)N1;
	float tex00 = tex2DLayered<float>(tex, t0.x, t0.y, (int)z);
	float tex10 = tex2DLayered<float>(tex, t1.x, t0.y, (int)z);
	float tex01 = tex2DLayered<float>(tex, t0.x, t1.y, (int)z);
	float tex11 = tex2DLayered<float>(tex, t1.x, t1.y, (int)z);
...
__global__ void interp(int interp_id, cudaTextureObject_t texfObj, cudaTextureObject_t texRObj, cudaTextureObject_t texflObj,
		       float *fo, float* x, float* y, int W, int Np,int N1,int N2, int ni, int* cids, int step2d)
{
	...
	float u = (x[tid]+0.5f)/N1;
	float v = (y[tid]+0.5f)/N2;
	
	switch(interp_id)//no overhead, all threads have the same way
	{ 		
		case 0: fo[tz*step2d+cids[tid]] += linearTex2D(texfObj, u, v, tz,N1,N2);break;
		case 1: fo[tz*step2d+cids[tid]] += linearTex2D(texRObj, u, v, tz,N1,N2);break;   
		case 2: fo[tz*step2d+cids[tid]] += linearTex2D(texflObj, u, v, tz,N1,N2);break;   
		case 3: fo[tz*step2d+cids[tid]] += cubicTex2D(texfObj, u, v, tz,N1,N2);break;
		case 4: fo[tz*step2d+cids[tid]] += cubicTex2D(texRObj, u, v, tz,N1,N2);break;   
		case 5: fo[tz*step2d+cids[tid]] += cubicTex2D(texflObj, u, v, tz,N1,N2);break;  
